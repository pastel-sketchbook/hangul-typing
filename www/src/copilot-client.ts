...
export class CopilotClient {
  ...
  async init(): Promise<CopilotStatus> {
    ...
    try {
      const response = await invoke<CommandResponse<CopilotStatus>>('copilot_init')
  ...
  async getStatus(): Promise<CopilotStatus> {
    ...
    try {
      const response = await invoke<CommandResponse<CopilotStatus>>('copilot_status')
   ...
   */
  async ask(prompt: string, context?: LearningContext): Promise<AssistantResponse | null> {
    if (!this.available) {
    ...
    try {
      const response = await invoke<CommandResponse<AssistantResponse>>('copilot_ask', {
        prompt,
   ...
   */
  async getHint(target: string, userInput: string, level: number): Promise<AssistantResponse | null> {
    if (!this.available) {
      return null
    }

    try {
      const response = await invoke<CommandResponse<AssistantResponse>>('copilot_hint', {
        target,
  ...
  async explain(text: string): Promise<AssistantResponse | null> {
    ...
    try {
      const response = await invoke<CommandResponse<AssistantResponse>>('copilot_explain', { text })
   ...
   */
  async analyzeMistake(expected: string, actual: string): Promise<AssistantResponse | null> {
    if (!this.available) {
      return null
    }

    try {
      const response = await invoke<CommandResponse<AssistantResponse>>('copilot_analyze_mistake', {
        expected,
        actual,
      })
