...
class HangulImeWasm implements IHangulImeWasm {
  ...
  commit(_field?: { value: string; selectionStart: number; selectionEnd: number }): string | null {
  ...
  private handleResult(
    ...
    if (this.debug) {
      console.log(
        `[HangulImeWasm] action=${action}, prev=U+${prevCodepoint.toString(16)}, curr=U+${currentCodepoint.toString(16)}`,
      )
    }
