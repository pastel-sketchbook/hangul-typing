...
import { getCopilotClient, type LearningContext } from './copilot-client'
...
export function recordMistake(expected: string, actual: string): void {
  gameContext.recentMistakes = [`${expected}→${actual}`, ...gameContext.recentMistakes.slice(0, 4)]
}
  ...
  switch (action) {
    case 'explain-target':
      if (gameContext.currentTarget) {
        ...
        if (response) {
          ...
          addMessage('Sorry, I could not get an explanation right now.', 'assistant', true)
        }
      } else {
    ...
    case 'get-hint':
      if (gameContext.currentTarget) {
        ...
        const hint = await client.getHint(gameContext.currentTarget, gameContext.userInput, gameContext.currentLevel)
        if (hint) {
          addMessage(hint.content, 'assistant')
        } else {
          addMessage('Sorry, I could not get a hint right now.', 'assistant', true)
        }
      } else {
    ...
    case 'how-to-type':
      if (gameContext.currentTarget) {
        ...
        if (response) {
          ...
          addMessage('Sorry, I could not get the typing sequence right now.', 'assistant', true)
        }
      } else {
...
async function askAssistant(prompt: string): Promise<void> {
  ...
  if (!client.isAvailable()) {
    addMessage('AI assistant is not available. Make sure GitHub Copilot CLI is installed.', 'assistant', true)
    return
  }
 ...
 */
function addMessage(content: string, sender: 'user' | 'assistant', isError = false): void {
  if (!elements) return
