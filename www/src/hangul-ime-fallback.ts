...
export interface IHangulIme {
  ...
  isEnabled(): boolean;
  reset(): void;
  handleKeyPress(event: { key: string }, field: { value: string; selectionStart: number; selectionEnd: number }): boolean;
...
export class HangulImeFallback implements IHangulIme {
  ...
  private compose(): number {
    ...
    return HANGUL_BASE + 
      (this.state.initial * MEDIAL_COUNT * FINAL_COUNT) + 
      (this.state.medial * FINAL_COUNT) + 
      this.state.final;
  }
  ...
  handleKeyPress(
    ...
    if (this.debug) {
      console.log(`[FallbackIME] State: ${JSON.stringify(this.state)}, char: ${String.fromCodePoint(this.compose() || 0x20)}`);
    }
